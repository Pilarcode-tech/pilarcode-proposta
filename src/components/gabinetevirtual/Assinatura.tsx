        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-bg-light border border-gray-200 rounded-3xl p-8 md:p-12 text-center">
            ...
            <p className="text-sm text-gray-500 leading-relaxed max-w-lg mx-auto mb-8">
              ...
              <strong className="text-gray-700">{gabinetevirtual.clienteCurto}</strong>{" "}
              indica o modelo escolhido (A, B ou C) e concorda com os termos,
              valores e condições descritos acima. Marcamos a chamada de
              fechamento da Fase 1 e o trabalho começa na semana seguinte, após a
              confirmação do aceite e o pagamento do sinal.
            </p>

            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-8">
              <a
                href={`https://wa.me/5511918958384?text=${encodeURIComponent(
                  "Olá! Aqui é o Bruno. Quero seguir com a proposta do Gabinete Virtual.",
                )}`}
                target="_blank"
            ...
            <div className="border-t border-gray-200 pt-6 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 text-xs text-gray-400">
              <span>Proposta válida por {gabinetevirtual.validade}</span>
              <span className="hidden sm:block">·</span>
              <span>Emissão: {gabinetevirtual.emissao}</span>
              <span className="hidden sm:block">·</span>
              <span>{gabinetevirtual.empresa}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-[#060554] p-6 lg:p-10 lg:px-20 text-white relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 lg:gap-0 pb-10 border-b border-white mt-10">
          <div className="flex flex-col gap-2">
            ...
            <a
              href="mailto:henrique.franca@pilarcode.com.br"
              className="text-sm text-white/80 hover:text-white transition-colors"
            >
              henrique.franca@pilarcode.com.br
            </a>
          </div>
          <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-2">
            <h3 className="font-semibold">Founder · Responsável Técnico</h3>
            <span className="text-sm text-white/80">Henrique França</span>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-2 mt-6 text-xs text-white/60">
          ...
          <p>Proposta emitida em {gabinetevirtual.emissao}</p>
        </div>
      </footer>
    </>
  );
