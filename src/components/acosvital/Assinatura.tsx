    ...
    <>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-white border border-gray-200 rounded-3xl p-8 md:p-12 text-center">
            ...
            <p className="text-sm text-gray-500 leading-relaxed max-w-lg mx-auto mb-8">
              Ao aceitar esta proposta, a{" "}
              <strong className="text-gray-700">{acosvital.clienteCurto}</strong>{" "}
              concorda com os termos, valores e condições descritos acima. O
              projeto tem início após a confirmação do aceite e a definição da
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-8">
              <a
                href={`https://wa.me/5511918958384?text=${encodeURIComponent(
                  `Olá! Aqui é da ${acosvital.clienteCurto}. Gostaríamos de aceitar a proposta do Dashboard Açosvital.`,
                )}`}
                target="_blank"
            ...
            <div className="border-t border-gray-200 pt-6 flex flex-col sm:flex-row items-center justify-center gap-4 sm:gap-6 text-xs text-gray-400">
              <span>Proposta válida por {acosvital.validade}</span>
              <span className="hidden sm:block">·</span>
              <span>Emissão: {acosvital.emissao}</span>
              <span className="hidden sm:block">·</span>
              <span>{acosvital.empresa}</span>
            </div>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-[#060554] p-6 lg:p-10 lg:px-20 text-white relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-2 mt-6 text-xs text-white/60">
          ...
          <p>Proposta emitida em {acosvital.emissao}</p>
        </div>
      </footer>
    </>
  );
